...
if((ArrayList<Integer>) reg_session.getAttribute("error_tips") != null){
	...
	out.print("<font color = \"red\"><img src=\"picture/Warning.png\" width=\"14\" height=\"14\">警告：エラー項目があります</font>");
}
...
<table>
	...
	<tr><td id=<%= tips.contains(0) ? "bg-red":"bg-blue"%>>従業員コード<%= tips.contains(0) ? "<img src=\"picture/Warning.png\" width=\"14\" height=\"14\">":""%></td><td><input type = "text" name = "emp_code" maxlength='4' <%= (String)reg_session.getAttribute("emp_code") != null ? " value = " + (String)reg_session.getAttribute("emp_code") : "" %> ></td></tr>
	...
	<tr><td id=<%= tips.contains(2) ? "bg-red":"bg-blue"%>>氏名（フリガナ）<%= tips.contains(2) ? "<img src=\"picture/Warning.png\" width=\"14\" height=\"14\">":""%></td><td>姓(セイ)<input type = "text" name = "l_kana_name" maxlength='16'  <%= (String)reg_session.getAttribute("l_kana_name") != null ? " value = " + (String)reg_session.getAttribute("l_kana_name") : "" %>>
	　名(メイ)<input type = "text" name = "f_kana_name" maxlength='16'  <%= (String)reg_session.getAttribute("f_kana_name") != null ? " value = " + (String)reg_session.getAttribute("f_kana_name") : "" %>></td></tr>
	<tr><td id=<%= tips.contains(3) ? "bg-red":"bg-blue"%>>性別<%= tips.contains(3) ? "<img src=\"picture/Warning.png\" width=\"14\" height=\"14\">":""%></td><td><input type = "radio" name = "sex" value = "0" <%= "0".equals(sex_str) ? " checked=\"checked\"" : "" %>>男　
						<input type = "radio" name = "sex" value = "1"  <%= "1".equals(sex_str) ? " checked=\"checked\"" : "" %>>女</td></tr>
	<tr><td id=<%= tips.contains(4) ? "bg-red":"bg-blue"%>>生年月日<%= tips.contains(4) ? "<img src=\"picture/Warning.png\" width=\"14\" height=\"14\">":""%></td><td><input type="date" name="birth_day"  max=<%= (String)request.getAttribute("today")%>  <%= (String)reg_session.getAttribute("birth_day_str") != null ? " value = " + (String)reg_session.getAttribute("birth_day_str") : "" %>>（手入力例：2000-01-01）</td></tr>
	<tr><td id=<%= tips.contains(5) ? "bg-red":"bg-blue"%>>所属部署<%= tips.contains(5) ? "<img src=\"picture/Warning.png\" width=\"14\" height=\"14\">":""%></td><td>
		 <%
...
<%= (String)reg_session.getAttribute("emp_code") != null ? " value = " + (String)reg_session.getAttribute("emp_code") : "" %>
		...
		<tr><td id=<%= tips.contains(6) ? "bg-red":"bg-blue"%>>入社日<%= tips.contains(6) ? "<img src=\"picture/Warning.png\" width=\"14\" height=\"14\">":""%></td><td><input type="date" name="emp_date"
